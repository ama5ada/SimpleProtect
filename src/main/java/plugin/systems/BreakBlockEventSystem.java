...
public class BreakBlockEventSystem extends EntityEventSystem<EntityStore, BreakBlockEvent> {
    public BreakBlockEventSystem(@Nonnull Class<BreakBlockEvent> eventType) {
        super(eventType);
    }
    @Override
    public void handle(int i, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk,
        ...
        if (ProtectionUtil.ShouldProtect(player, worldName, EVENT_TYPE.BLOCK_BREAK)) breakBlockEvent.setCancelled(true);
    }
